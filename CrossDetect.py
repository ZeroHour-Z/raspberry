...
def getCross(img):
  ...
  for c in con:
    ...
    if(s>3000):#面积去除小干扰
      ...
      if((float(w)/h>0.8 and float(w)/h<1.2) and abs(len(ct)-12)<7 and float(sr)/s<3.5):#外接矩形宽高比和角点数量
        # img=cv2.rectangle(img,(x+w,y+h),(x,y),(255,255,0),1,0)
        con2.append(ct)
      else:
        cv2.putText(img,f'{float(w)/h:.3f},{abs(len(ct)-12)},{float(sr)/s:.3f}',(ct[0][0][0],ct[0][0][1]),0,1.5,(255,0,0),2)
