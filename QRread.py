...
def getQRCode(img,QR_ind=11):
  ...
  # out=open('t.txt','w')
  for i in range(l):
    if(hie[0][i][2]!=-1 and abs(len(con[i])-4)<1 and hie[0][hie[0][i][2]][0]==-1):#有子轮廓并且点少（接近正方形）且只有一个
      have_son.append(i)
  # print(have_son)
  ...
  for i in range(len(have_son)):
    if(hie[0][hie[0][have_son[i]][2]][2]!=-1 and hie[0][hie[0][hie[0][have_son[i]][2]][2]][2]==-1 and hie[0][hie[0][have_son[i]][2]][0]==-1):#子轮廓还有子轮廓且孙子没有子轮廓且子轮廓只有一个
  ...
  if(len(res)==3):
    ...
    for i in range(3):
      center.append([(res[i][0][0][0]+res[i][1][0][0]+res[i][2][0][0]+res[i][3][0][0])/4,(res[i][0][0][1]+res[i][1][0][1]+res[i][2][0][1]+res[i][3][0][1])/4])
      for j in range(4):
    ...
    bound=np.array([[[left-10,down+10]],[[right+10,down+10]],[[right+10,top-10]],[[left-10,top-10]]])
